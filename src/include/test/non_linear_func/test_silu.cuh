...
void silu_test()
{
    cout << "Task: test the SiLU function in CKKS scheme. " << endl;
...
#pragma omp parallel num_threads(nthreads)
    {
        ...
        for (int i = 0; i < num_col; ++i)
        {
            output[i] = silu(enc_ecd_x[i], context, relin_keys, secret_key, stream);
        }
        cudaStreamSynchronize(stream.get_stream());
    }

    gettimeofday(&tend1, NULL);
    double silu_time = tend1.tv_sec - tstart1.tv_sec + (tend1.tv_usec - tstart1.tv_usec) / 1000000.0;
    cout << "silu time = " << silu_time << endl;
    cout << "silu time (amortized) = " << silu_time / num_col << endl;
    cout << "Modulus chain index for silu: " << context.get_context_data(output[0].params_id()).chain_depth() << endl;
    append_csv_row("../results.csv", "silu", silu_time);
    // decrypt
    cout << "Decrypt + decode result of intermediate_silu: " << endl;
