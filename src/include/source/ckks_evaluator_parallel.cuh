...
namespace moai
{
  ...
  public:
    ...
    // Encode with stream
    // =============================
    inline void encode(vector<double> values, size_t chain_index, double scale,
    ...
    // =============================
    template <typename T,
              typename = std::enable_if_t<std::is_same<std::remove_cv_t<T>, double>::value ||
                                          std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
    inline void decode(PhantomPlaintext &plain, vector<T> &values,
                       const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      encoder->decode(*context, plain, values, stream_wrapper);
    }
  };
  ...
  public:
    ...
    inline void square(PhantomCiphertext &ct, PhantomCiphertext &dest,
                       const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      multiply(ct, ct, dest, stream_wrapper);
    }

    inline void square_inplace(PhantomCiphertext &ct,
                               const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      multiply_inplace(ct, ct, stream_wrapper);
    }
                         ...
                         const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      if (&ct2 == &dest)
      {
        multiply_inplace(dest, ct1, stream_wrapper);
      }
      else
      {
        dest = ct1;
        multiply_inplace(dest, ct2, stream_wrapper);
      }
    }
                               ...
                               const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      ...
      dest = ::multiply_plain(*context, ct, plain, stream_wrapper);
    }
                         ...
                         const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      ...
      add(cts[0], cts[1], dest, stream_wrapper);
      for (size_t i = 2; i < size; i++)
      {
        add_inplace(dest, cts[i], stream_wrapper);
      }
    }
    ...
    {
      dest = ct;
      sub_plain_inplace(dest, plain, stream_wrapper);
    }
                    ...
                    const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      if (&ct2 == &dest)
      {
        sub_inplace(dest, ct1, stream_wrapper);
        negate_inplace(dest, stream_wrapper);
      }
      else
      {
        dest = ct1;
        sub_inplace(dest, ct2, stream_wrapper);
      }
    }
    ...
    {
      dest = ct;
      complex_conjugate_inplace(dest, galois_keys, stream_wrapper);
    }
    ...
    {
      dest = ct;
      transform_to_ntt_inplace(dest, stream_wrapper);
    }
    ...
    {
      dest = ct;
      multiply_const_inplace(dest, value, stream_wrapper);
    }
                                       ...
                                       const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      ...
      vector<double> values(encoder->slot_count(), value);
      encoder->encode(*context, values, ct.scale(), const_plain, 1, stream_wrapper);
      mod_switch_to_inplace(const_plain, ct.params_id(), stream_wrapper);
      multiply_plain_inplace(ct, const_plain, stream_wrapper);
    }
    ...
    {
      dest = ct;
      add_const_inplace(dest, value, stream_wrapper);
    }
                                  ...
                                  const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      ...
      encoder->encode(*context, values, ct.scale(), const_plain, 1, stream_wrapper);
      mod_switch_to_inplace(const_plain, ct.params_id(), stream_wrapper);
      add_plain_inplace(ct, const_plain, stream_wrapper);
    }
                                  ...
                                  const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      if (&ct2 == &dest)
      {
        add_inplace_reduced_error(dest, ct1, stream_wrapper);
      }
      else
      {
        dest = ct1;
        add_inplace_reduced_error(dest, ct2, stream_wrapper);
      }
    }
    ...
    {
      dest = ct1;
      sub_inplace_reduced_error(dest, ct2, stream_wrapper);
    }
                                       ...
                                       const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      if (&ct2 == &dest)
      {
        multiply_inplace_reduced_error(dest, ct1, relin_keys, stream_wrapper);
      }
      else
      {
        dest = ct1;
        multiply_inplace_reduced_error(dest, ct2, relin_keys, stream_wrapper);
      }
    }
    ...
    {
      dest = ct;
      multiply_vector_inplace_reduced_error(dest, values, stream_wrapper);
    }
                                                      ...
                                                      const cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream)
    {
      ...
      values.resize(encoder->slot_count(), 0.0);
      encoder->encode(*context, values, ct.scale(), plain, 1, stream_wrapper);
      mod_switch_to_inplace(plain, ct.params_id(), stream_wrapper);
      multiply_plain_inplace(ct, plain, stream_wrapper);
    }
